...
class CRCServer(object):
    ...
    def setup_server_socket(self):
        ...
        self.sel.register(self.server_sock, self.events, None)     
        self.my_sock_data = {}
        self.my_sock_data ['id '] = 73
        self.my_sock_data [' write_buffer '] = b''
    ...
    def connect_to_server(self):
        ...
        remote_sock.connect((self.connect_to_host_addr, int(self.connect_to_port)))
    ...
    def accept_new_connection(self, select_key):
        ...
        # self.sel.register(conn,selectors.EVENT_READ, None)
        pass
    ...
    def handle_io_device_events(self, io_device, event_mask):
        # TODO: Implement the above functionality
        hand = socket(AF_INET, SOCK_STREAM)
         # Get a reference to the socket
        hand.connect((self.connect_to_host_addr, int(self.connect_to_port)))
         # Get the data associated with the socket

        # Check if a read operation can be performed
        if selectors.EVENT_READ:
            self.msg = self.hand.recv (1024) # Read data from the socket
            self.handle_messages(self.my_sock_data , self.msg ) # Pass the received msg and my_sock_data to

    
        # Check if a write operation can be performed
        if event_mask & selectors.EVENT_WRITE :
            if len ( self.my_sock_data [' write_buffer ']) > 0: # Check to make sure we have data to write
                self.hand.send ( self.my_sock_data [' write_buffer '])
        # Send the contents of write_buffer
                self.my_sock_data [' write_buffer '] = b''  
        ready = self.sel.select(0.5)
