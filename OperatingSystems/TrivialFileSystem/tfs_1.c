...
void tfs_list_directory(){
  ...
  for( fd = FIRST_VALID_FD; fd < N_DIRECTORY_ENTRIES; fd++ ){
    ...
    if( directory[fd].status == UNUSED ){
      ...
      printf( "%s, currently closed, %d bytes in size, read=%d, write=%d\n",
        directory[fd].name, directory[fd].size, directory[fd].read_perm, directory[fd].write_perm  );
    }else if( directory[fd].status == OPEN ){
      printf( "%s, currently open, %d bytes in size, read=%d, write=%d\n",
        directory[fd].name, directory[fd].size, directory[fd].read_perm, directory[fd].write_perm );
    }else{
      printf( "*** status error\n" );
    }
...
unsigned int tfs_create( char *name ){
  ...
  directory[file_descriptor].read_perm = 0;
  directory[file_descriptor].write_perm = 0;
