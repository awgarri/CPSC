...
void __attribute__ ((destructor)) allocator_cleanup(void) {
...
int* create_page( size_t size) {

	int* actual_addr = Mmap(NULL, size + sizeof(T), PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
...
int* create_cache_page( size_t size) {

	int* actual_addr = Mmap(NULL, PAGESIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
	T *temp = (T*)actual_addr;	

	int cache_id = find_cache_size(size);
	*temp = create_header( cache_id);
	temp->available = ( (PAGESIZE - sizeof(T)) / cache_id);			

	//set the free_list ptr to the user_space 
	int* user_space = actual_addr + (sizeof(T) >> 3);
	temp->free_list = user_space;

	int *temp_ptr = user_space;
...
void* realloc( void* ptr, size_t new_size) {
	...
	T *header =  ( T*) ( (int) ptr & 0xFFFFFFFFF000);
